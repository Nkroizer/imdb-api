...
title.get("/:id", async (c) => {
  ...
  try {
    ...
    let rawHtml = await apiRequestRawHtml(`https://m.imdb.com/title/${id}`);
    ...
    // imdb link
    response.imdb = `https://m.imdb.com/title/${id}`;
...
title.get("/:id/season/:seasonId", async (c) => {
  ...
  try {
    const html = await apiRequestRawHtml(
      `https://m.imdb.com/title/${id}/episodes/?season=${seasonId}`
    );
    ...
    const response = Object.assign(
      {
        ...
        imdb: `https://m.imdb.com/title/${id}/episodes/?season=${seasonId}`,
        season_id: seasonId,
      },
      parsed
    );
